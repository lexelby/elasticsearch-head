	...
	es.ui.ClusterOverview = es.ui.Page.extend({
		...
		redraw: function(command) {
			if(command === "reset") {
				...
				this.cluster.get("_cluster/state/nodes", this._clusterNodes_handler);
        ...
        function timgroupSort(a,b) {
          var asplit = a.split("-");
          var bsplit = b.split("-");

          var alogstash = asplit[0] == "logstash";
          var blogstash = bsplit[0] == "logstash";

          if (!alogstash && !blogstash) {
            return compare(a,b);
          }
          ...
          return compare(atier, btier);
				...
				indexNames.sort(timgroupSort).forEach(function(name) {
		...
		_nodeCol_template: function(node) {
			return {
				tag: "COL",
				cls: "clusterOverview-node" + (node.master_node ? " master": "")
			}
		},
		_nodeHeader_template: function(node) {
			return {
				tag: "TH",
				children: node.name === "Unassigned" ?
					[
      ...
      var _alias_template = function(alias) {
        return {
             tag: "DIV",
             css: { background: "#" + "9ce9c7fc9".substr(hashCode(alias)%7,3) },
             cls: "clusterOverview-hasAlias",
             text: alias,
             children: [
               { tag: 'SPAN',
			...
			return index.name ?  {
				tag: "TR",
				children: [
					{ tag: "TD", cls: (closed ? "close" : ""), children: [
					{ tag: "DIV", cls: "clusterOverview-title", text: index.name },
          { tag: "DIV", children: index.metadata.aliases.map(_alias_template, this) },
            { tag: "DIV", text: line1 },
						{ tag: "DIV", text: line2 },
						{ tag: "DIV", children: [
							new acx.ui.MenuButton({
				...
				].concat(index.routings.map(this._routing_template, this))
			} : { tag: "TR", child : { tag: "TD" } }
		},
      ...
      function compare(a, b) {
        ...
        first = first.cluster.name.split(" ").reverse().join(" ");
        second = second.cluster.name.split(" ").reverse().join(" ");
        return compare(first, second);
			}
			var nodes = cluster.nodes.sort(nodeNameCmp);
			for (var j=1; j<indices.length; j++) {
				indices[j].routings = [];
				for (var i=0; i<nodes.length; i++) {
					indices[j].routings[i] = nodes[i].routings[j-1];
				}
			}
			return {
				tag: "TABLE",
				cls: "clusterOverview-cluster",
				children: [
					{
						tag: "COLGROUP",
						children: [ { tag: "COL" } ].concat(nodes.map(this._nodeCol_template, this))
					},
					{
						tag: "THEAD",
						child: {
							tag: "TR",
							children: [ { tag: "TH" } ].concat(nodes.map(this._nodeHeader_template, this))
						}
					},
					{
						tag: "TBODY",
						children: indices.map(this._index_template, this)
					}
				]
			};
		},
